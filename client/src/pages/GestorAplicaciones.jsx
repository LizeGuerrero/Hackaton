...
const GestorAplicaciones = () => {
  ...
  useEffect(() => {
    loadAplicaciones(); // Carga todas las aplicaciones
    loadEtiquetas(); // Carga todas las etiquetas
  }, []);

  // Cargar aplicaciones desde el servidor
  const loadAplicaciones = async () => {
    try {
      const data = await getAplicaciones(); // Obtiene las aplicaciones usando el servicio
      setAplicaciones(data); // Actualiza el estado con las aplicaciones obtenidas
    } catch (error) {
      console.error('Error al cargar las aplicaciones:', error); // Maneja errores de carga
    }
  };
